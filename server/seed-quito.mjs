/**
 * seed-quito.mjs
 * Crea el tipo "Peluches", todos sus campos y las 6 máquinas del archivo
 * "Recaudacion QUITO.xlsx".  Ejecutar con:
 *   node server/seed-quito.mjs
 * (el servidor API debe estar corriendo en :3001)
 */
...
const LUGAR = { nombre: 'JFC San Lorenzo', direccion: 'San Lorenzo, Paraguay', lat: -25.3395, lng: -57.5089 };

const TIPO_DESC = 'Peluches';

// Campos en orden de aparición en el formulario.
// constant: no aparece en el form, disponible como {key} en fórmulas.
const CAMPOS = [
  // ── Constante global ─────────────────────────────────────────────────────
...
async function main() {
  ...
  for (const campo of CAMPOS) {
    if (existingKeys.has(campo.key)) {
      console.log(`  · Skipping existing campo: ${campo.key}`);
      skipped++;
      continue;
    }
    ...
    console.log(`  ✅ Campo: ${campo.key} (${campo.grupo})`);
    created++;
  }
  console.log(`Campos: ${created} creados, ${skipped} ya existían.`);
  ...
  for (const id of MACHINES) {
    if (existingIds.has(id)) {
      console.log(`  · Skipping existing machine: ${id}`);
      mSkipped++;
      continue;
    }
  ...
  console.log(`Máquinas: ${mCreated} creadas, ${mSkipped} ya existían.`);
